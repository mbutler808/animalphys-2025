...
Pyrimidal neurons are the most numerous excitatory neurons in the cerebral cortex [@Beckers:2011]. They transform the sum of synaptic inputs into patterns of action potentials which may travel to other regions of the brain, or to muscle targets far away in the body to signal motor behavior. When we perform EEG we are measuring the activities of many (100s of thousands) of pyrimidal neurons as they go about processing information and coordinating activity. 

![Pyrimidal neurons are the most common neuron in the cerebral cortex. The name comes from the triangular shape of their soma. UC Regents Davis campus, [CC BY 3.0](https://creativecommons.org/licenses/by/3.0), via Wikimedia Commons](../../images/lab11-pyrimidal-neurons.png){#fig-neurons}. 

Brain waves can be classified by frequency bands. This means that they have characteristic patterns of oscillation over time (@fig-brainwaves). One of the earliest uses of EEG was characterizing brain activity during sleep, which led to the definition of alpha(7-13Hz), beta (13-30Hz), theta (3.5-7Hz) and delta (0.5-3.5Hz) waves [@fig-eegpatterns; @Vibell:2023]. The dominance of the different frequences changes with different stages of sleep. Although these waves were originally defined during sleep, they do occur with other behavioral states and brain activities.
...
: EEG patterns, frequency ranges, and associated behavioral states. Delta, Theta, Alpha, and Beta definitions are from early sleep studies. Slow and fast gamma are associated with cognitive processing. [@Vibell:2023] {#tbl-eegpatterns}

Alpha waves (8-12 Hz) can be measured spontaneously and are prominent over the occipital (visual) region of the brain.  When our eyes are open and are in an alert state, alpha waves tend to be suppressed. They are prominent when the eyes are closed and we are relaxed (@tbl-eegpattern). You will do an "eyes open/eyes closed" experiment today. Beta waves increase when we are alert and are associated with focus or problem-solving. Gamma waves are associated with cognitive processing. 
